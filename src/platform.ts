...
export class RoborockMatterPlatform implements DynamicPlatformPlugin {
  ...
  private readonly devices: Map<string, ActiveDevice> = new Map();
  ...
  constructor(
    ...
    this.log.info(
      `Xiaomi Roborock Matter platform loaded on Homebridge ${api.serverVersion}`,
    );
    this.log.debug(
      `Matter available=${api.isMatterAvailable()} enabled=${api.isMatterEnabled()}`,
    );
  ...
  configureAccessory(_accessory: PlatformAccessory) {
    this.log.debug(
      'Ignoring cached accessory (Matter devices are self-managed)',
    );
  }

  configureMatterAccessory(accessory: MatterAccessory) {
    this.log.info(
      `Loaded cached Matter accessory "${accessory.displayName}" (${accessory.UUID})`,
    );
  ...
  private async discoverDevices() {
    ...
    this.log.debug(
      `Matter status: available=${this.api.isMatterAvailable()} enabled=${this.api.isMatterEnabled()} api=${this.api.matter ? 'present' : 'missing'}`,
    );

    if (!this.api.isMatterEnabled() || !this.api.matter) {
      this.log.error(
        'Matter is not enabled for this Homebridge instance. Enable bridge.matter before using this plugin.',
      );
      return;
    }

    const platformConfig = this.config as unknown as RoborockPlatformConfig;

    if (!platformConfig.devices?.length) {
      this.log.warn(
        'No devices configured. Add devices to your config.json under the RoborockMatter platform.',
      );
      return;
    }

    this.log.info(
      `Configured Roborock devices: ${platformConfig.devices.length}. Cached Matter accessories: ${this.matterAccessories.size}.`,
    );
  ...
  private async startDevice(deviceConfig: RoborockDeviceConfig) {
    const { name, ip, token, pollInterval = 10_000 } = deviceConfig;
    ...
    if (!/^[0-9a-fA-F]{32}$/.test(token)) {
      this.log.warn(
        `Device "${name}" has a token that is not 32 hex characters. Connection will probably fail.`,
      );
    }

    this.log.info(
      `Setting up device "${name}" at ${ip} with poll interval ${pollInterval} ms`,
    );
    this.log.debug(`Device "${name}" token length: ${token.length}`);

    const client = new RoborockClient(ip, token, this.log);
    ...
    try {
      ...
      this.log.warn(
        `Could not detect model for "${name}" before Matter registration: ${err}. Using fallback model until the robot connects.`,
      );
    }
    ...
    try {
      ...
      this.log.info(
        `Initial state for "${name}" (${client.getModel()}): status=${state.status}, battery=${state.batteryLevel}%, fan=${state.fanSpeed}%, error=${state.errorCode}`,
      );
    } catch (err) {
      this.log.warn(
